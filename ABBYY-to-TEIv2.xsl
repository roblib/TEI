...
<xsl:stylesheet
	...
	<xsl:template match="page">
		<div type="page" xmlns="http://www.tei-c.org/ns/1.0">
			<xsl:attribute name="n"><xsl:value-of select="position()-1"/></xsl:attribute><xsl:text>&#10;</xsl:text>
			<xsl:comment>This is scanned page number <xsl:value-of select="position()-1"/> of the work <xsl:value-of select="$marcid"/>.</xsl:comment><xsl:text>&#10;</xsl:text>
	...
	<xsl:template match="block[@blockType='Picture']">
		<div xmlns="http://www.tei-c.org/ns/1.0"><xsl:text>&#10;</xsl:text>
